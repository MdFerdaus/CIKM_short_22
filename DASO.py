...
full_data = pd.read_csv("data_300NM.csv")
data = full_data
test_nohold = data[data["holding"] == 0].sample(n=960, random_state=0) # ===================================
test_hold = data[data["holding"] == 1].sample(n=960, random_state=0)
train_nohold = data[data["holding"] == 0].drop(test_nohold.index)
train_hold = data[data["holding"] == 1].drop(test_hold.index)
...
feature =['lat', 'long', 'heading', 'altitude','groundspeed',
         ...
         'dep_rwy_02L_cap', 'dep_rwy_20R_cap', 'dep_rwy_02C_cap','dep_rwy_20C_cap', 'STAR_grp']

X_test = test_set[feature]
y_test = test_set['holding']

X_train = train_set[feature]
y_train = train_set['holding']
...
######  SMOTE oversampling (scikit learn)
# oversample = SMOTE()
# oversample = ADASYN()
...
train_data = torch.Tensor(X_train).float()  # transform to torch tensor
...
train_loader = torch.utils.data.DataLoader(aug_dataset, batch_size=batch_size, shuffle=False, num_workers=2)
...
balanced_loader = torch.utils.data.DataLoader(train_dataset, batch_size=len(train_dataset), shuffle=False,
                                              num_workers=2)

do_data, do_class = iter(balanced_loader).next()
test_data, test_labels = iter(test_loader).next()

if cuda:
    do_data = Variable(do_data.type(Tensor))
    do_class = do_class.cuda()
...
for iter in range(3):
    ############ model defines here
    gen = gen_maps(do_data, do_class, n_classes=num_class)
    ...
    # ##### optimiser defines here
    # #### train feature + reconstrutions
    optim_G = torch.optim.Adam(gen.parameters(), lr=base_lr, betas=(0.5, 0.9999))
    # #### train feature + cot loss
    ...
    optim_cot = optim.Adam(mlp_class.parameters(), lr=base_lr, betas=(0.5, 0.9999))
    ...
    for epoch in range(epochs):
        ...
        # ###### generated samples for fake data
            optim_cot.zero_grad()
            gen.eval()
            augment_samples = real_label  #### oversample is for different structure
            ...
            aug_ce_loss = compl_loss(aug_class_score, augment_samples)
    ...
    print("Max_GM", gm_max)
    print("roc_score_max", roc_score_max)
    all_max_ACSA.append(deepcopy(acsa_max))
    all_max_GM.append(deepcopy(gm_max))

print("all_max_ACSA", all_max_ACSA)
print("all_max_GM", all_max_GM)
# print("all_roc_score_max", all_roc_score_max)
